...
export default function Hero() {
  ...
  const stats = [
    { n: "2000", l: lang === "mn" ? "Үүсгэн байгуулагдсан" : "Founded" },
    { n: "II", l: lang === "mn" ? "Үе дамжсан гар" : "Generations" },
    {
      n: "20",
      l: lang === "mn" ? "Гар хийц бүтээгдэхүүн" : "Pieces in catalog",
    },
  ...
  return (
    <section
      id="top"
      className="relative min-h-screen w-full overflow-hidden bg-[var(--paper)]">
      ...
      <div className="relative max-w-[1600px] mx-auto px-6 lg:px-12 pt-32 lg:pt-40 pb-20 min-h-screen flex flex-col">
        <div className="flex-1 flex flex-col justify-center max-w-[760px]">
          ...
          <h1 className="serif font-light tracking-tight leading-[0.92] text-[56px] sm:text-[88px] md:text-[112px] lg:text-[132px]">
            ...
            <span className="block reveal" style={{ transitionDelay: "240ms" }}>
              {t.title[2] ?? ""}
            </span>
          </h1>

          <div
            className="mt-12 lg:mt-14 max-w-md reveal"
            style={{ transitionDelay: "380ms" }}>
            <p className="text-[15px] leading-relaxed text-[var(--muted)] mb-8">
              {t.sub}
            </p>
            <div className="flex flex-wrap gap-4">
              <a
                href="#collection"
                className="group inline-flex items-center gap-3 bg-[var(--ink)] text-[var(--paper)] px-7 py-4 text-[13px] tracking-wide hover:bg-[var(--ink)]/90 transition-colors">
                <span>{t.cta1}</span>
                <svg
                  width="14"
                  height="14"
              ...
              <a
                href="#showroom"
                className="inline-flex items-center gap-3 px-7 py-4 text-[13px] tracking-wide border hairline-strong hover:bg-[var(--ink)] hover:text-[var(--paper)] transition-colors">
                <span>{t.cta2}</span>
              </a>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-12 pt-12 mt-12 border-t hairline reveal">
          {stats.map((s, i) => (
            <div key={i}>
              <div className="serif text-[40px] lg:text-[52px] font-light leading-none">
                {s.n}
              </div>
